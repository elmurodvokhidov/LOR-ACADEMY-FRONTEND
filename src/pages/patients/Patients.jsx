...
const Patients = () => {
    ...
    const exportToExcel = () => {
        ...
        const header = ['Ism (FIO)', 'Jinsi', 'Tug\'ilgan sanasi', 'Telefon', 'Kasallik turi', 'Shifokor', 'To\'lov so\'mmasi', 'To\'ladi', 'Chegirma', 'Passport', 'Email manzili',];
    ...
    const searchedPatients = patients?.filter(patient => patient?.fullname?.toLowerCase().includes(search.trim().toLowerCase()) || patient?.symptom?.name?.toLowerCase().includes(search.trim().toLowerCase()) || patient?.doctor?.fullname?.toLowerCase().includes(search.trim().toLowerCase()));
        ...
        if (
            ...
            try {
                ...
                if (!newPatient._id) {
                    ...
                    await service.updatePatient(newPatient._id, newPatient);
    ...
    return (
        <div className="container">
            <div className="flex items-center justify-between mb-8">
                <div className="flex flex-col justify-start text-sm pc:text-lg">
                    <h1 className="text-xl pc:text-3xl">Bemorlar ro'yhati</h1>
                    <p>Miqdor <span className="inline-block w-4 h-px mx-1 align-middle bg-black"></span> <span>{searchedPatients?.length}</span></p>
                </div>
                <div className="flex items-center gap-4">
                    <div className="w-80 h-10 flex items-center rounded-xl px-4 pc:py-3 bg-gray-100">
                        <input
            ...
            <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
                <table className="w-full text-sm pc:text-xl text-left rtl:text-right text-gray-500">
                    <thead className="text-xs pc:text-lg text-gray-700 uppercase bg-gray-50">
                        <tr>
                            ...
                            <th scope="col" className="px-6 py-3">
                                <div className="flex items-center gap-2">
                                    <button onClick={openManyDeleteModal} className="size-6 pc:size-8 flex items-center justify-center text-sm pc:text-xl border rounded-full text-red-600 border-red-600 hover:bg-red-600 hover:text-white transition-all duration-300">
                                        <Basket />
                                    </button>
                                    <Link to={"/admin/patients-reports"} className="size-6 pc:size-8 flex items-center justify-center text-xl border rounded-full text-blue-600 border-blue-600 hover:bg-blue-600 hover:text-white transition-all duration-300">
                                        <TbReportAnalytics />
                                    </Link>
                                </div>
                            </th>
                    ...
                    <tbody>
                        {pagePatients?.length > 0 ?
                            pagePatients.map(patient => (
                                <tr key={patient._id} className="odd:bg-white even:bg-gray-50 border-b">
                                    ...
                                    <td className="px-6 py-4 font-semibold">
                                        {patient?.symptom?.price?.toLocaleString()}
                                    </td>
                                    <td className="px-6 py-4 font-semibold">
                                        {patient?.discount}%
                                    </td>
                            ...
                            <tr><td className="py-12 text-center text-lg pc:text-xl odd:bg-white even:bg-gray-50" colSpan={10}>Bemor mavjud emas</td></tr>}
                    </tbody>
                </table>
            </div>
