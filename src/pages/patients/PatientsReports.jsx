...
const PatientsReports = () => {
    ...
    const exportToExcel = () => {
        ...
        const header = ['Kasallik turi', 'Shifokor ismi', 'To\'lov so\'mmasi (so\'mda)', 'Chegirma (%)', 'To\'ladi', 'Bemor ismi', 'Tug\'ilgan sanasi', 'Jinsi', 'Telefon raqami', 'Passport seriya raqami', 'Email manzili'];
                ...
                <table className="min-w-full text-sm pc:text-lg bg-white border-2 border-gray-300">
                    <thead>
                        <tr>
                            ...
                            <th className="px-4 py-2 border-2">To'lov so'mmasi</th>
                            <th className="px-4 py-2 border-2">Chegirma (%)</th>
                            <th className="px-4 py-2 border-2">To'ladi</th>
                        ...
                        {Object.keys(groupedPatients).map(symptom => {
                            return groupedPatients[symptom].map(patient => (
                                <tr key={patient?._id}>
                                    ...
                                    <td className="px-4 py-2 border-2">{patient?.discount}</td>
                                    <td className="px-4 py-2 border-2">{patient?.amount?.toLocaleString()}</td>
